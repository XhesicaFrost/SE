...
public class Rider {
    ...
    public void decrementActiveOrders() {
        System.out.println("[DEBUG] 调用 decrementActiveOrders()");
        System.out.println("[DEBUG] 当前 activeOrders=" + this.activeOrders + ", status=" + this.status);
    
        if (this.activeOrders > 0) {
            this.activeOrders--;
            System.out.println("[DEBUG] 减少后 activeOrders=" + this.activeOrders);
    
            if (this.activeOrders == 0 && this.status == RiderStatus.DELIVERING) {
                System.out.println("[DEBUG] 满足转换条件，将状态从 DELIVERING 改为 ONLINE");
                this.status = RiderStatus.ONLINE;
            }
        }
    
        System.out.println("[DEBUG] 最终 activeOrders=" + this.activeOrders + ", status=" + this.status);
    }
